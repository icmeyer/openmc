...
!===============================================================================
  ...
  !===============================================================================
  ! WRITE_DOUBLE_5D writes double precision 5-D array data
  !===============================================================================

  subroutine write_double_5D(group_id, name, buffer, indep)
    integer(HID_T), intent(in) :: group_id
    character(*), intent(in)           :: name      ! name of data
    real(8),      intent(in), target   :: buffer(:,:,:,:,:) ! data to write
    logical,      intent(in), optional :: indep   ! independent I/O
    ...
    dims(:) = shape(buffer)
    if (present(indep)) then
      call write_double_5D_explicit(group_id, dims, name, buffer, indep)
    else
      call write_double_5D_explicit(group_id, dims, name, buffer)
    end if
  end subroutine write_double_5D

  subroutine write_double_5D_explicit(group_id, dims, name, buffer, indep)
    integer(HID_T), intent(in) :: group_id
    integer(HSIZE_T), intent(in) :: dims(5)
    character(*), intent(in)           :: name      ! name of data
    real(8),      intent(in), target   :: buffer(dims(1),dims(2),dims(3),dims(4),dims(5))
    logical,      intent(in), optional :: indep   ! independent I/O
    ...
    call h5screate_simple_f(5, dims, dspace, hdf5_err)
    call h5dcreate_f(group_id, trim(name), H5T_NATIVE_DOUBLE, &
          dspace, dset, hdf5_err)
    f_ptr = c_loc(buffer)
